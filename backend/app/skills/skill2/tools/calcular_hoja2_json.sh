...
RUNTIME="$SCRIPT_DIR/../runtime/calcular_hoja2_runtime.py"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      cat <<'USAGE'
Uso:
  backend/app/skills/skill2/tools/calcular_hoja2_json.sh --peso "800g" --columna "C" [--xlsx "/ruta.xlsx"] [--pretty]
  backend/app/skills/skill2/tools/calcular_hoja2_json.sh --peso "0.8kg" --servicio "America Del Sur" [--xlsx "/ruta.xlsx"] [--pretty]
USAGE
      exit 0 ;;
    *)
