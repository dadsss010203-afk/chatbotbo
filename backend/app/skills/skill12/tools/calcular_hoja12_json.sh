...
RUNTIME="$SCRIPT_DIR/../runtime/calcular_hoja12_runtime.py"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      cat <<'USAGE'
Uso:
  backend/app/skills/skill12/tools/calcular_hoja12_json.sh --peso "800g" --columna "C" [--xlsx "/ruta.xlsx"] [--pretty]
  backend/app/skills/skill12/tools/calcular_hoja12_json.sh --peso "0.8kg" --servicio "América del Sur / Destino A" [--xlsx "/ruta.xlsx"] [--pretty]
USAGE
      exit 0 ;;
    *)
...
if [[ -z "$precio" && "$output" != *=* ]]; then
  output_norm="$(echo "$output" | tr '[:upper:]' '[:lower:]')"
  if [[ "$output_norm" == *"peso fuera de rango"* || "$output_norm" == *"precio vacio"* ]]; then
    if [[ $pretty -eq 1 ]]; then
      printf '{\n  "ok": false,\n  "error": "Peso fuera de rango para este tarifario",\n  "error_code": "out_of_range",\n  "raw": "%s"\n}\n' "$(json_escape "$output")"
    else
      printf '{"ok":false,"error":"Peso fuera de rango para este tarifario","error_code":"out_of_range","raw":"%s"}\n' "$(json_escape "$output")"
    fi
    exit 1
  fi
  if [[ $pretty -eq 1 ]]; then
