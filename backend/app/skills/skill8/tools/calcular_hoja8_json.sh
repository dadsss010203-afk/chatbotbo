...
RUNTIME="$SCRIPT_DIR/../runtime/calcular_hoja8_runtime.py"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      cat <<'USAGE'
Uso:
  backend/app/skills/skill8/tools/calcular_hoja8_json.sh --peso "800g" --columna "C" [--xlsx "/ruta.xlsx"] [--pretty]
  backend/app/skills/skill8/tools/calcular_hoja8_json.sh --peso "0.8kg" --servicio "América Del Sur / Destino A" [--xlsx "/ruta.xlsx"] [--pretty]
USAGE
      exit 0 ;;
    *)
